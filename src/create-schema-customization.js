...
exports.createSchemaCustomization = async (
  { cache, actions, schema, reporter, createNodeId },
  configOptions
) => {
  let contentTypes;
  ...
  if (configOptions.enableSchemaGeneration) {
    ...
    try {
      ...
      fieldConfig = getGatsbyImageFieldConfig(
        async (image, options) =>
          resolveGatsbyImageData({ image, options, cache, reporter }),
        {
      ...
      if (error.code === 'MODULE_NOT_FOUND') {
        reporter.info(
          `Gatsby plugin image is required to use new gatsby image plugin's feature. Please check https://github.com/contentstack/gatsby-source-contentstack#the-new-gatsby-image-plugin for more help.`
        );
      }
    }

    createTypes([
      schema.buildObjectType(contentTypeSchema),
      schema.buildObjectType(assetTypeSchema),
    ]);
    ...
    await Promise.all([
      await cache.set(
        `${typePrefix}_${configOptions.api_key}_references`,
        references
      ),
      await cache.set(`${typePrefix}_${configOptions.api_key}_groups`, groups),
      await cache.set(
        `${typePrefix}_${configOptions.api_key}_file_fields`,
        fileFields
      ),
      await cache.set(
        `${typePrefix}_${configOptions.api_key}_json_rte_fields`,
        jsonRteFields
      ),
    ]);
  }
